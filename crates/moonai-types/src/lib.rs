...
pub fn deterministic_respawn(seed: u64, step: u64, entity_id: u32) -> (f32, f32) {
    let mut h = seed;
    h = h.wrapping_mul(0x9e3779b97f4a7c15).wrapping_add((step << 32) | entity_id as u64);
    h ^= h >> 30;
...
mod tests {
    ...
    #[test]
    fn deterministic_respawn_different_steps() {
        let (x1, y1) = deterministic_respawn(42, 100, 5);
