...
void print_genome_diff(const moonai::Genome &a, const moonai::Genome &b) {
  ...
  std::printf("Nodes:  A=%d  B=%d\n", static_cast<int>(nodes_a.size()),
              static_cast<int>(nodes_b.size()));
  ...
  for (const auto &c : a.connections())
    conns_a[c.innovation] = &c;
  for (const auto &c : b.connections())
    conns_b[c.innovation] = &c;

  int enabled_a = static_cast<int>(
      std::count_if(a.connections().begin(), a.connections().end(),
                    [](const auto &c) { return c.enabled; }));
  int enabled_b = static_cast<int>(
      std::count_if(b.connections().begin(), b.connections().end(),
                    [](const auto &c) { return c.enabled; }));
  ...
  for (const auto &[innov, ca] : conns_a) {
    ...
    if (it == conns_b.end()) {
      std::printf("  - Removed conn #%u (%u->%u w=%.4f%s)\n", innov,
                  ca->in_node, ca->out_node, ca->weight,
                  ca->enabled ? "" : " [disabled]");
    } else {
      ...
      if (std::abs(dw) > 0.01f || ca->enabled != cb->enabled) {
        std::printf("  ~ Conn #%u (%u->%u): w %.4f->%.4f (delta %.4f)%s\n",
                    innov, ca->in_node, ca->out_node, ca->weight, cb->weight,
                    dw,
  ...
  for (const auto &[innov, cb] : conns_b) {
    if (!conns_a.count(innov)) {
      std::printf("  + Added conn #%u (%u->%u w=%.4f%s)\n", innov, cb->in_node,
                  cb->out_node, cb->weight, cb->enabled ? "" : " [disabled]");
    }
  }
}
} // namespace
...
int main(int argc, const char *argv[]) {
  ...
  // Accepts genome JSON files (output of genome.to_json()) or checkpoint JSON
  // files. For checkpoints, the best genome (population[0]) is used.
  if (!args.compare_a.empty()) {
    auto load_genome = [](const std::string &path) -> std::string {
      std::string content = read_file(path);
      if (content.empty())
        return "";
      // If it looks like a checkpoint (has "population" key), extract first
      // genome
      try {
    ...
    print_genome_diff(moonai::Genome::from_json(json_a),
                      moonai::Genome::from_json(json_b));
    return 0;
  }
  ...
  if (args.run_all) {
    ...
    for (auto &[name, cfg] : all_configs) {
      ...
      if (args.max_generations_override != 0)
        cfg.max_generations = args.max_generations_override;
      ...
      int gen = 0;
      while (g_running &&
             (cfg.max_generations == 0 || gen < cfg.max_generations)) {
        simulation.reset();
  ...
  if (!args.experiment_name.empty()) {
    ...
    if (it == all_configs.end()) {
      spdlog::error(
          "Experiment '{}' not found. Use --list to see available experiments.",
    ...
    if (args.headless) {
      spdlog::warn("Multiple experiments found. Use --experiment or --all. "
                   "Using first.");
      config = all_configs.begin()->second;
      selected_experiment = all_configs.begin()->first;
    } else {
  ...
  if (args.max_generations_override != 0)
    config.max_generations = args.max_generations_override;
  ...
  if (!args.resume_path.empty()) {
    if (p_evolution->load_checkpoint(args.resume_path, rng)) {
      ...
      spdlog::error("Failed to load checkpoint '{}'. Exiting.",
                    args.resume_path);
      return 1;
    }
  }
  ...
  auto logger = std::make_unique<moonai::Logger>(config.output_dir, config.seed,
                                                 output_name);
  ...
  bool has_multi_config =
      all_configs.size() > 1 && args.experiment_name.empty();
  if (!headless) {
  ...
  auto reinit_for_experiment = [&](const std::string &exp_name) {
    ...
    logger = std::make_unique<moonai::Logger>(config.output_dir, config.seed,
                                              output_name);
    logger->initialize(config);
    generation = 0;
  ...
  int max_gen = (args.max_generations_override != 0)
                    ? args.max_generations_override
  ...
  while (g_running) {
    ...
    if (headless || visualization.is_fast_forward()) {
      ...
      while (tick < config.generation_ticks && g_running) {
        ...
        if (visualization.in_experiment_select_mode())
          break;
...
#ifdef MOONAI_ENABLE_CUDA
        // CUDA path: GPU with CPU fallback
        std::vector<moonai::Vec2> gpu_actions(p_simulation->agents().size(),
                                              {0.0f, 0.0f});
        bool visual_gpu_ready = p_evolution->prepare_gpu_generation();
        for (int s = 0; s < steps; ++s) {
          bool used_gpu = false;
          if (visual_gpu_ready &&
              p_evolution->infer_actions_gpu(*p_simulation, gpu_actions)) {
            used_gpu = true;
            MOONAI_PROFILE_MARK_GPU_USED(true);
            for (size_t i = 0;
                 i < p_simulation->agents().size() && i < networks.size();
                 ++i) {
              if (!p_simulation->agents()[i]->alive())
                continue;
              p_simulation->apply_action(i, gpu_actions[i], dt);
            }
          } else {
            // GPU was ready but inference failed - disable for this generation
          ...
          if (p_simulation->alive_prey() == 0 ||
              p_simulation->alive_predators() == 0)
            break;
          if (tick >= config.generation_ticks)
            break;
        }
#else
        ...
        for (int s = 0; s < steps; ++s) {
          ...
          MOONAI_PROFILE_SCOPE(moonai::ProfileEvent::CpuEvalTotal);
          for (size_t i = 0;
               i < p_simulation->agents().size() && i < networks.size(); ++i) {
            if (!p_simulation->agents()[i]->alive())
              continue;
        ...
        if (sel >= 0 && sel < static_cast<int>(networks.size()) &&
          ...
          networks[sel]->activate(
              p_simulation->get_sensors(static_cast<size_t>(sel)).to_vector());
      ...
      // If we broke out for experiment selector, skip the rest of the
      // generation
      if (visualization.in_experiment_select_mode())
        continue;
    ...
    // Collect metrics
    auto m = metrics.collect(generation, p_evolution->population(),
    ...
    spdlog::info("Gen {:>4d}: best={:.3f} avg={:.3f} species={} alive=({}/{}) "
                 "complexity={:.1f}",
                 generation, m.best_fitness, m.avg_fitness, m.num_species,
    ...
    if (headless && max_gen > 0) {
      ...
      std::printf("\r  Gen %d/%d [%d%%]  best=%.3f  avg=%.3f  species=%d   ",
                  generation, max_gen, pct, m.best_fitness, m.avg_fitness,
    ...
    // Fire on_generation_end hook (may return config overrides)
    if (lua_runtime.callbacks().has_on_generation_end) {
      moonai::GenerationStats gs{generation,
                                 m.best_fitness,
                                 m.avg_fitness,
                                 m.num_species,
                                 p_simulation->alive_predators(),
