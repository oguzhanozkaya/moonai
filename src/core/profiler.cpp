...
const char *profile_event_description(ProfileEvent event) {
  switch (event) {
  case ProfileEvent::GenerationTotal:
    return "Wall-clock generation duration measured in headless mode.";
  case ProfileEvent::BuildNetworks:
    return "Time spent rebuilding neural networks from genomes.";
  case ProfileEvent::PrepareGpuGeneration:
    return "Time spent uploading GPU network state before a generation.";
  case ProfileEvent::GpuSensorFlatten:
    return "Time spent preparing CPU-side sensor input buffers for the legacy "
           "GPU inference path.";
  case ProfileEvent::GpuResidentSensorBuild:
    return "Time spent building resident GPU-side sensor inputs from "
           "GPU-resident world state.";
  case ProfileEvent::GpuPackInputs:
  ...
  case ProfileEvent::GpuLaunch:
    return "Time spent running GPU neural inference for the legacy hybrid "
           "path.";
  case ProfileEvent::GpuResidentTick:
    return "Time spent running the resident GPU simulation tick after "
           "inference.";
  case ProfileEvent::GpuStartUnpack:
  ...
  case ProfileEvent::CpuEvalTotal:
    return "Total CPU inference path time across a generation.";
  case ProfileEvent::CpuSensorBuild:
  ...
  case ProfileEvent::SimulationTick:
    return "Accumulated time spent inside simulation ticks.";
  case ProfileEvent::RebuildSpatialGrid:
    return "Time spent rebuilding the agent spatial grid.";
  case ProfileEvent::RebuildFoodGrid:
    return "Time spent rebuilding the food spatial grid.";
  case ProfileEvent::AgentUpdate:
    return "Time spent updating agent age and per-tick state.";
  case ProfileEvent::ProcessEnergy:
    return "Time spent applying per-tick energy drain.";
  case ProfileEvent::ProcessFood:
  ...
  case ProfileEvent::DeathCheck:
    return "Time spent marking dead agents after tick processing.";
  case ProfileEvent::CountAlive:
  ...
  case ProfileEvent::RemoveStagnantSpecies:
    return "Time spent pruning stagnant species.";
  case ProfileEvent::Reproduce:
    return "Time spent producing the next generation population.";
  case ProfileEvent::Logging:
    return "Time spent writing generation logs.";
  case ProfileEvent::TickCallback:
    return "Time spent executing optional tick callbacks.";
  case ProfileEvent::Count:
    return "";
  }
  return "";
...
const char *profile_counter_description(ProfileCounter counter) {
  switch (counter) {
  case ProfileCounter::TicksExecuted:
    return "Number of simulation ticks completed.";
...
const char *gpu_stage_timing_description(GpuStageTiming stage) {
  switch (stage) {
  case GpuStageTiming::ResidentTickBinRebuildPre:
    return "Device elapsed time for the resident tick's pre-inference spatial "
           "bin rebuild.";
  case GpuStageTiming::ResidentTickSensorBuild:
    return "Device elapsed time for resident sensor input construction.";
  case GpuStageTiming::ResidentTickInference:
    return "Device elapsed time for resident neural inference.";
  case GpuStageTiming::ResidentTickMovement:
    return "Device elapsed time for resident movement integration.";
  case GpuStageTiming::ResidentTickBinRebuildPost:
    return "Device elapsed time for the resident tick's post-movement spatial "
           "bin rebuild.";
  case GpuStageTiming::ResidentTickPreyFood:
    return "Device elapsed time for resident prey-food interactions.";
  case GpuStageTiming::ResidentTickPredatorAttack:
    return "Device elapsed time for resident predator attack interactions.";
  case GpuStageTiming::ResidentTickRespawn:
    return "Device elapsed time for resident food respawn.";
...
void Profiler::finish_generation(const GenerationProfileMeta &meta) {
  ...
  if (generation_active_) {
    ...
    current_durations_ns_[enum_index(ProfileEvent::GenerationTotal)].store(
        std::chrono::duration_cast<std::chrono::nanoseconds>(generation_end -
                                                             generation_start_)
            .count(),
        std::memory_order_relaxed);
    generation_active_ = false;
...
void Profiler::finish_run(std::int64_t run_total_ns) {
  ...
  profile["notes"] = {
      "Profiling is only supported in headless mode so generation_total is not "
      "polluted by GUI rendering or pause time.",
      "Event durations are stored in milliseconds in both per-generation and "
      "summary sections.",
      "Events with measurement='accumulated' sum repeated scoped timings "
      "within a generation.",
      "Path-specific events can remain zero for generations that never execute "
      "that path.",
      "cpu_generation_count and gpu_generation_count are non-exclusive; a "
      "fallback generation can increment both counts.",
      "Fields ending with nonzero_generation_count count generations where the "
      "recorded value was greater than zero.",
      "Use nonzero_generation_count and avg_ms_per_nonzero_generation when "
      "judging optional events.",
      "gpu_stage_timings use device elapsed timestamps collected inside the "
      "resident GPU tick sequence.",
      "gpu_stage_timings decompose gpu_resident_tick and are diagnostic "
      "timings, not top-level generation wall-clock events."};
  ...
  std::array<int, enum_index(ProfileEvent::Count)>
      active_duration_generations{};
  std::array<int, enum_index(ProfileCounter::Count)>
      active_counter_generations{};
  std::array<int, enum_index(GpuStageTiming::Count)>
      active_gpu_stage_generations{};
  ...
  summary["path_count_note"] =
      "cpu_generation_count and gpu_generation_count are non-exclusive; "
      "fallback generations can count toward both.";
        ...
        {"avg_per_generation",
         generation_records_.empty()
             ...
             : static_cast<double>(total_counters[i]) /
                   static_cast<double>(generation_records_.size())},
        {"nonzero_generation_count", active_count},
...
const char *profile_event_name(ProfileEvent event) {
  ...
  case ProfileEvent::PrepareGpuGeneration:
    return "prepare_gpu_generation";
  case ProfileEvent::GpuSensorFlatten:
    return "gpu_sensor_flatten";
  case ProfileEvent::GpuResidentSensorBuild:
    return "gpu_resident_sensor_build";
  case ProfileEvent::GpuPackInputs:
    return "gpu_pack_inputs";
  case ProfileEvent::GpuLaunch:
    return "gpu_launch";
  case ProfileEvent::GpuResidentTick:
    return "gpu_resident_tick";
  case ProfileEvent::GpuStartUnpack:
  ...
  case ProfileEvent::Speciate:
    return "speciate";
  case ProfileEvent::RemoveStagnantSpecies:
    return "remove_stagnant_species";
  case ProfileEvent::Reproduce:
    return "reproduce";
  case ProfileEvent::Logging:
    return "logging";
