...
std::vector<moonai::gpu::GpuAgentState>
build_gpu_agent_states(const moonai::SimulationManager &sim, int agent_count) {
  std::vector<moonai::gpu::GpuAgentState> states(
      static_cast<size_t>(agent_count));
  for (int i = 0; i < agent_count; ++i) {
    ...
    states[static_cast<size_t>(i)] = moonai::gpu::GpuAgentState{
        ...
        static_cast<unsigned int>(
            agent->type() == moonai::AgentType::Predator ? 0 : 1),
        static_cast<unsigned int>(agent->alive() ? 1 : 0)};
  }
  return states;
}

std::vector<moonai::gpu::GpuFoodState>
build_gpu_food_states(const moonai::SimulationManager &sim) {
  ...
  for (size_t i = 0; i < food.size(); ++i) {
    states[i] = moonai::gpu::GpuFoodState{
        food[i].position.x, food[i].position.y,
        static_cast<unsigned int>(food[i].active ? 1 : 0)};
  }
  return states;
}

void sync_gpu_state_to_simulation(
    const std::vector<moonai::gpu::GpuAgentState> &agent_states,
...
// ──────────────────────────────────────────────── Extracts the CSR-packed flat
// arrays from NeuralNetwork objects into the GpuNetworkData struct. Lives here
// (in moonai_evolution) because it needs NeuralNetwork internals, keeping
// moonai_gpu free of moonai_evolution dependency.
  ...
  for (int i = 0; i < n; ++i) {
    ...
    int num_out = static_cast<int>(
        std::count_if(nodes.begin(), nodes.end(), [](const auto &node) {
          return node.type == moonai::NodeType::Output;
        }));
  ...
  for (int i = 0; i < total_pop; ++i) {
    ...
    for (const auto &in_node : g.nodes()) {
      ...
      for (const auto &out_node : g.nodes()) {
        ...
        g.add_connection({in_node.id, out_node.id, rng_.next_float(-1.0f, 1.0f),
                          true, innov});
      }
    }
...
void EvolutionManager::enable_gpu(bool use_gpu) {
  ...
  if (use_gpu_) {
    if (num_inputs_ > 0 && !population_.empty()) {
      ...
      if (!gpu_batch_->ok()) {
        spdlog::warn(
            "GpuBatch allocation failed; falling back to CPU inference");
        gpu_batch_.reset();
        use_gpu_ = false;
      }
    }
...
bool EvolutionManager::prepare_gpu_generation() {
  MOONAI_PROFILE_SCOPE(ProfileEvent::PrepareGpuGeneration);
...
bool EvolutionManager::infer_actions_gpu(const SimulationManager &sim,
  ...
  int agent_count =
      static_cast<int>(std::min(sim.agents().size(), networks_.size()));
  if (agent_count <= 0) {
    return true;
  }
...

  {
    ...
    gpu_batch_->upload_tick_state_async(
        agent_states.data(), agent_count, food_states.data(),
        static_cast<int>(food_states.size()), sim.spatial_grid().cols(),
        sim.spatial_grid().rows(), sim.spatial_grid().cell_size(),
        sim.food_grid().cols(), sim.food_grid().rows(),
  ...
  {
    MOONAI_PROFILE_SCOPE(ProfileEvent::GpuStartUnpack);
...

  {
    ...
    for (int i = 0; i < agent_count; ++i) {
      ...
      if (num_outputs_ >= 2) {
        actions[i] = Vec2{flat_out[i * num_outputs_ + 0] * 2.0f - 1.0f,
...
void EvolutionManager::evaluate_generation(SimulationManager &sim) {
  ...
  std::vector<float> sensor_values(
      static_cast<size_t>(agent_count * num_inputs_), 0.0f);
  std::vector<float> output_values(
      static_cast<size_t>(agent_count * num_outputs_), 0.0f);
  ...
  if (prepare_gpu_generation()) {
    ...
    if (!tick_callback_ && !config_.tick_log_enabled) {
      ...
      for (int tick = 0; tick < config_.generation_ticks; ++tick) {
        {
          ...
          gpu_batch_->launch_resident_inference_tick_async(
              dt, static_cast<float>(config_.grid_width),
              static_cast<float>(config_.grid_height),
              config_.boundary_mode == BoundaryMode::Clamp,
              config_.energy_drain_per_tick, config_.target_fps,
              config_.food_pickup_range, config_.attack_range,
              config_.initial_energy, config_.energy_gain_from_food,
              config_.energy_gain_from_kill, config_.food_respawn_rate,
              config_.seed, tick);
        }
      ...
      if (use_gpu_) {
        ...
        if (!gpu_batch_->ok()) {
          spdlog::warn(
              "GPU resident simulation failed; falling back to CPU inference");
          gpu_batch_.reset();
          use_gpu_ = false;
        } else {
          ...
          if (!gpu_batch_->ok()) {
            spdlog::warn(
                "GPU state download failed; falling back to CPU inference");
            gpu_batch_.reset();
            use_gpu_ = false;
          } else {
    ...
    for (int tick = 0; tick < config_.generation_ticks; ++tick) {
      ...
      {
        MOONAI_PROFILE_SCOPE(ProfileEvent::ApplyActions);
      ...
      if (tick_callback_) {
        MOONAI_PROFILE_SCOPE(ProfileEvent::TickCallback);
...
#pragma omp parallel for schedule(dynamic) if (MOONAI_OPENMP_ENABLED)
      for (int i = 0; i < agent_count; ++i) {
        ...
        sim.get_sensors(static_cast<size_t>(i))
            .write_to(sensor_values.data() +
                      static_cast<size_t>(i * num_inputs_));
      }
    }
...
#pragma omp parallel for schedule(dynamic) if (MOONAI_OPENMP_ENABLED)
      for (int i = 0; i < agent_count; ++i) {
        ...
        networks_[i]->activate_into(sensor_buf, num_inputs_, output_buf,
                                    num_outputs_);

        if (num_outputs_ >= 2) {
          actions[i] =
              Vec2{output_buf[0] * 2.0f - 1.0f, output_buf[1] * 2.0f - 1.0f};
        }
      }
    }
...
void EvolutionManager::evolve() {
  ...
  int target_species =
      std::max(2, (config_.predator_count + config_.prey_count) / 10);
  ...
  config_.compatibility_threshold =
      std::max(config_.compatibility_threshold, 0.5f);
...
void EvolutionManager::compute_fitness(const SimulationManager &sim) {
  MOONAI_PROFILE_SCOPE(ProfileEvent::ComputeFitness);
  ...
  for (size_t i = 0; i < population_.size() && i < agents.size(); ++i) {
    ...
    float energy_ratio =
        std::max(0.0f, agent->energy()) / config_.initial_energy;
    ...
    float fitness;
    if (use_lua) {
      fitness = lua_runtime_->call_fitness(age_ratio, kills_or_food,
                                           energy_ratio, alive_bonus,
                                           dist_ratio, complexity, config_);
    } else {
...
float EvolutionManager::default_fitness(float age_ratio, float kills_or_food,
  ...
  float f = config_.fitness_survival_weight * age_ratio +
            ...
            config_.fitness_energy_weight * energy_ratio + alive_bonus +
...
void EvolutionManager::speciate() {
  ...
  // Clear stale member pointers first (they point into the previous
  // generation's population which was freed in reproduce()), then update the
  // representative from the now-empty members (keeps the existing
  // representative if no members).
  for (auto &s : species_) {
    s.clear_members();
...
void EvolutionManager::remove_stagnant_species() {
  MOONAI_PROFILE_SCOPE(ProfileEvent::RemoveStagnantSpecies);
  if (species_.size() <= 2)
    return; // Always keep at least 2 species

  species_.erase(std::remove_if(species_.begin(), species_.end(),
                                [this](const Species &s) {
                                  return s.is_stagnant(
...
void EvolutionManager::reproduce() {
  MOONAI_PROFILE_SCOPE(ProfileEvent::Reproduce);
  ...
  float total_adjusted = std::accumulate(
      species_.begin(), species_.end(), 0.0f, [](float sum, const auto &s) {
        return sum + s.total_adjusted_fitness();
      });
  ...
  for (size_t si = 0; si < species_.size(); ++si) {
    ...
    for (int i = 1; i < offspring_count; ++i) {
      ...
      if (members.size() == 1 || !rng_.next_bool(config_.crossover_rate)) {
        // Mutation only
        child =
            *members[rng_.next_int(0, static_cast<int>(members.size()) - 1)];
      } else {
        // Crossover + mutation
...
void EvolutionManager::save_checkpoint(const std::string &path,
  ...
  for (const auto &genome : population_) {
    ...
    auto it = std::find_if(
        species_.begin(), species_.end(), [&genome](const auto &s) {
          return std::any_of(s.members().begin(), s.members().end(),
                             [&genome](const auto *m) { return m == &genome; });
        });
    if (it != species_.end()) {
      entry["species_id"] = it->id();
    }
...
bool EvolutionManager::load_checkpoint(const std::string &path, Random &rng) {
  ...
  for (const auto &se : j["species"]) {
    ...
    Species s(rep);
    // Override id (Species constructor auto-increments, but we set next_id
    // above) We need to reconstruct with the correct id — create and then fix
    // by building a matching species entry. Since set_next_species_id was
    // called before loop, we already incremented; we need to handle ids by
    // rebuilding species_ from scratch. The simplest approach: build species
  ...
  spdlog::info("Checkpoint loaded: '{}' (resuming from gen {})", path,
               generation_);
  return true;
}

const Genome *EvolutionManager::genome_at(int idx) const {
  if (idx < 0 || idx >= static_cast<int>(population_.size()))
    return nullptr;
