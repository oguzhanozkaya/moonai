...
__global__ void kernel_count_population_cells(
...
__global__ void kernel_scatter_population_cells(
    const float *__restrict__ pos_x, const float *__restrict__ pos_y,
  ...
  entries[slot] = GpuPopulationEntry{static_cast<unsigned int>(idx), pos_x[idx],
                                     pos_y[idx], 0.0f};
}
...
__global__ void kernel_scatter_food_cells(
  ...
  const int slot = atomicAdd(&cell_offsets[cy * grid_cols + cx], 1);
  entries[slot] =
      GpuFoodEntry{static_cast<unsigned int>(idx), food_pos_x[idx],
                   food_pos_y[idx], 0.0f};
}
...
__global__ void kernel_build_sensors(
  ...
  for (int dy_cell = -cells_to_check; dy_cell <= cells_to_check; ++dy_cell) {
    for (int dx_cell = -cells_to_check; dx_cell <= cells_to_check; ++dx_cell) {
      ...
      for (int slot = predator_cell_offsets[cell];
           slot < predator_cell_offsets[cell + 1]; ++slot) {
      ...
      for (int slot = prey_cell_offsets[cell]; slot < prey_cell_offsets[cell + 1];
           ++slot) {
...
__global__ void kernel_claim_food(
    const float *__restrict__ prey_pos_x, const float *__restrict__ prey_pos_y,
    const uint32_t *__restrict__ prey_alive,
    const int *__restrict__ food_cell_offsets,
    const GpuFoodEntry *__restrict__ food_entries,
    int *__restrict__ food_consumed_by, int prey_count, int grid_cols,
    int grid_rows, float grid_cell_size, float pickup_range, float world_width,
    float world_height) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void kernel_finalize_food(float *__restrict__ prey_energy,
                                     const uint32_t *__restrict__ prey_alive,
...
__global__ void kernel_claim_combat(
    const float *__restrict__ predator_pos_x,
    const float *__restrict__ predator_pos_y,
    const uint32_t *__restrict__ predator_alive,
    const int *__restrict__ prey_cell_offsets,
    const GpuPopulationEntry *__restrict__ prey_entries,
    int *__restrict__ prey_claimed_by, int predator_count, int grid_cols,
    int grid_rows, float grid_cell_size, float interaction_range,
...
__global__ void kernel_finalize_combat(
    float *__restrict__ predator_energy, const uint32_t *__restrict__ predator_alive,
    uint32_t *__restrict__ predator_kill_counts,
    uint32_t *__restrict__ prey_alive, const int *__restrict__ prey_claimed_by,
    int prey_count, float energy_gain_from_kill) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= prey_count || prey_alive[idx] == 0) {
    return;
  }
...
GpuBatch::GpuBatch(std::size_t max_predators, std::size_t max_prey,
                   std::size_t max_food)
    : predator_buffer_(max_predators), prey_buffer_(max_prey),
      food_buffer_(max_food) {
  init_cuda_resources();
}
...
void GpuBatch::ensure_spatial_grid_capacity(std::size_t cell_count) {
  ...
  CUDA_CHECK(
      cudaMalloc(&d_predator_cell_counts_, (cell_count + 1) * sizeof(int)));
  CUDA_CHECK(
      cudaMalloc(&d_predator_cell_offsets_, (cell_count + 1) * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&d_predator_cell_write_offsets_, cell_count * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&d_predator_grid_entries_,
                        predator_buffer_.capacity() * sizeof(GpuPopulationEntry)));
  ...
  CUDA_CHECK(cudaMalloc(&d_prey_grid_entries_,
                        prey_buffer_.capacity() * sizeof(GpuPopulationEntry)));
  ...
  CUDA_CHECK(cudaMalloc(&d_food_grid_entries_,
                        food_buffer_.capacity() * sizeof(GpuFoodEntry)));

  grid_cell_capacity_ = cell_count;
}

void GpuBatch::upload_async(std::size_t predator_count, std::size_t prey_count,
...
void GpuBatch::download_async(std::size_t predator_count, std::size_t prey_count,
                              std::size_t food_count) {
  const cudaStream_t stream = static_cast<cudaStream_t>(stream_);
...
void GpuBatch::launch_build_sensors_async(const GpuStepParams &params,
  ...
  const cudaStream_t stream = static_cast<cudaStream_t>(stream_);
  const int predator_blocks =
      (static_cast<int>(predator_count) + kThreadsPerBlock - 1) / kThreadsPerBlock;
  const int prey_blocks =
      (static_cast<int>(prey_count) + kThreadsPerBlock - 1) / kThreadsPerBlock;
...

  {
    ...
    CUDA_CHECK(cudaMemsetAsync(d_predator_cell_counts_, 0,
                               (cell_count + 1) * sizeof(int), stream));
    CUDA_CHECK(cudaMemsetAsync(d_prey_cell_counts_, 0,
                               (cell_count + 1) * sizeof(int), stream));
    CUDA_CHECK(cudaMemsetAsync(d_food_cell_counts_, 0,
                               (cell_count + 1) * sizeof(int), stream));

    if (predator_count > 0) {
      kernel_count_population_cells<<<predator_blocks, kThreadsPerBlock, 0,
                                      stream>>>(predator_buffer_.device_positions_x(),
                                                predator_buffer_.device_positions_y(),
                                                predator_buffer_.device_alive(),
                                                d_predator_cell_counts_,
                                                static_cast<int>(predator_count),
    ...
    if (prey_count > 0) {
      kernel_count_population_cells<<<prey_blocks, kThreadsPerBlock, 0, stream>>>(
          ...
          prey_buffer_.device_alive(), d_prey_cell_counts_,
          static_cast<int>(prey_count), grid_cols_, grid_rows_, grid_cell_size_);
    }
    if (food_count > 0) {
      kernel_count_food_cells<<<food_blocks, kThreadsPerBlock, 0, stream>>>(
          food_buffer_.device_positions_x(), food_buffer_.device_positions_y(),
          food_buffer_.device_active(), d_food_cell_counts_,
    ...
    thrust::exclusive_scan(thrust::cuda::par.on(stream), d_predator_cell_counts_,
                           d_predator_cell_counts_ + cell_count + 1,
                           d_predator_cell_offsets_);
    thrust::exclusive_scan(thrust::cuda::par.on(stream), d_prey_cell_counts_,
                           d_prey_cell_counts_ + cell_count + 1,
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_predator_cell_write_offsets_,
                               d_predator_cell_offsets_, cell_count * sizeof(int),
                               cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_prey_cell_write_offsets_, d_prey_cell_offsets_,
    ...
    if (predator_count > 0) {
      kernel_scatter_population_cells<<<predator_blocks, kThreadsPerBlock, 0,
                                        stream>>>(
          predator_buffer_.device_positions_x(),
          predator_buffer_.device_positions_y(), predator_buffer_.device_alive(),
          d_predator_cell_write_offsets_, d_predator_grid_entries_,
    ...
    if (prey_count > 0) {
      kernel_scatter_population_cells<<<prey_blocks, kThreadsPerBlock, 0, stream>>>(
          prey_buffer_.device_positions_x(), prey_buffer_.device_positions_y(),
          prey_buffer_.device_alive(), d_prey_cell_write_offsets_,
          d_prey_grid_entries_, static_cast<int>(prey_count), grid_cols_,
          grid_rows_, grid_cell_size_);
    }
    if (food_count > 0) {
      kernel_scatter_food_cells<<<food_blocks, kThreadsPerBlock, 0, stream>>>(
          food_buffer_.device_positions_x(), food_buffer_.device_positions_y(),
          food_buffer_.device_active(), d_food_cell_write_offsets_,
...

  {
    ...
    if (predator_count > 0) {
      CUDA_CHECK(cudaMemsetAsync(predator_buffer_.device_brain_outputs(), 0,
                                 predator_count * 2 * sizeof(float), stream));
    }
    if (prey_count > 0) {
      CUDA_CHECK(cudaMemsetAsync(prey_buffer_.device_brain_outputs(), 0,
                                 prey_count * 2 * sizeof(float), stream));
    }
  }
...
void GpuBatch::launch_post_inference_async(const GpuStepParams &params,
  ...
  if (predator_count > 0) {
    kernel_update_vitals<<<predator_blocks, kThreadsPerBlock, 0, stream>>>(
        predator_buffer_.device_energy(), predator_buffer_.device_age(),
        predator_buffer_.device_alive(), static_cast<int>(predator_count),
        params.energy_drain_per_step, params.max_age);
    CUDA_CHECK(cudaMemsetAsync(predator_buffer_.device_kill_counts(), 0,
                               predator_count * sizeof(uint32_t), stream));
  }
  if (prey_count > 0) {
    kernel_update_vitals<<<prey_blocks, kThreadsPerBlock, 0, stream>>>(
        prey_buffer_.device_energy(), prey_buffer_.device_age(),
        prey_buffer_.device_alive(), static_cast<int>(prey_count),
        params.energy_drain_per_step, params.max_age);
    CUDA_CHECK(cudaMemsetAsync(prey_buffer_.device_claimed_by(), 0x7f,
                               prey_count * sizeof(int), stream));
  }
  if (food_count > 0) {
    CUDA_CHECK(cudaMemsetAsync(food_buffer_.device_consumed_by(), 0x7f,
  ...
  if (prey_count > 0 && food_count > 0) {
    kernel_claim_food<<<prey_blocks, kThreadsPerBlock, 0, stream>>>(
        prey_buffer_.device_positions_x(), prey_buffer_.device_positions_y(),
        prey_buffer_.device_alive(), d_food_cell_offsets_, d_food_grid_entries_,
        food_buffer_.device_consumed_by(), static_cast<int>(prey_count),
    ...
    kernel_finalize_food<<<food_blocks, kThreadsPerBlock, 0, stream>>>(
        prey_buffer_.device_energy(), prey_buffer_.device_alive(),
        food_buffer_.device_active(), food_buffer_.device_consumed_by(),
        static_cast<int>(food_count), params.energy_gain_from_food);
  }
  ...
  if (predator_count > 0 && prey_count > 0) {
    kernel_claim_combat<<<predator_blocks, kThreadsPerBlock, 0, stream>>>(
        ...
        predator_buffer_.device_alive(), d_prey_cell_offsets_,
        d_prey_grid_entries_, prey_buffer_.device_claimed_by(),
        static_cast<int>(predator_count), grid_cols_, grid_rows_, grid_cell_size_,
        params.interaction_range, params.world_width, params.world_height);
    kernel_finalize_combat<<<prey_blocks, kThreadsPerBlock, 0, stream>>>(
        predator_buffer_.device_energy(), predator_buffer_.device_alive(),
        predator_buffer_.device_kill_counts(), prey_buffer_.device_alive(),
        prey_buffer_.device_claimed_by(), static_cast<int>(prey_count),
