...
std::vector<AgentId> SpatialGrid::query_radius(Vec2 position,
                                               float radius) const {
  std::vector<AgentId> result;
...
void SpatialGrid::flatten(std::vector<int> &cell_offsets,
                          std::vector<SpatialGrid::FlatEntry> &entries) const {
  cell_offsets.clear();
  entries.clear();
  ...
  std::size_t total_entries = std::accumulate(
      cells_.begin(), cells_.end(), std::size_t{0},
      [](std::size_t sum, const auto &cell) { return sum + cell.size(); });
