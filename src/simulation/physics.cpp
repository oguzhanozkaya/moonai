...
SensorInput
Physics::build_sensors(const Agent &agent,
...
SensorInput Physics::build_sensors_from_candidates(
  ...
  if (nearest_pred_dist_sq < std::numeric_limits<float>::max()) {
    ...
    s.nearest_predator_angle =
        normalize_angle(nearest_pred_dir.x, nearest_pred_dir.y);
  }

  // Nearest prey
  if (nearest_prey_dist_sq < std::numeric_limits<float>::max()) {
    s.nearest_prey_dist = std::sqrt(nearest_prey_dist_sq) / vision;
    s.nearest_prey_angle =
        normalize_angle(nearest_prey_dir.x, nearest_prey_dir.y);
  }
  ...
  if (nearest_food_dist_sq < std::numeric_limits<float>::max()) {
    ...
    s.nearest_food_angle =
        normalize_angle(nearest_food_dir.x, nearest_food_dir.y);
  }
  ...
  s.local_predator_density =
      std::clamp(local_predators / max_density, 0.0f, 1.0f);
...
Physics::process_attacks(std::vector<std::unique_ptr<Agent>> &agents,
  ...
  for (size_t i = 0; i < agents.size(); ++i) {
    ...
    grid.query_radius_into(agents[i]->position(), attack_range,
                           nearby_agent_ids[i]);
  }
