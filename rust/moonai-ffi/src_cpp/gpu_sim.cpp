...
int sim_init(
    ...
    ctx->set_error("CUDA is not available. MoonAI requires GPU for simulation. Use CMake build with MOONAI_USE_CMAKE=1.");
    return 0;
}
...
int sim_prepare_step(SimulationContext* ctx) {
    ...
    ctx->set_error("CUDA is not available");
    return 0;
}
...
int sim_step(SimulationContext* ctx) {
    ...
    ctx->set_error("CUDA is not available. MoonAI requires GPU for simulation.");
    return 0;
}
...
void sim_get_predator_state(
    ...
    if (out_pos_x) *out_pos_x = nullptr;
    if (out_pos_y) *out_pos_y = nullptr;
    if (out_vel_x) *out_vel_x = nullptr;
    if (out_vel_y) *out_vel_y = nullptr;
    if (out_energy) *out_energy = nullptr;
    if (out_age) *out_age = nullptr;
    if (out_alive) *out_alive = nullptr;
...
void sim_get_food_state(
    ...
    if (out_pos_x) *out_pos_x = nullptr;
    if (out_pos_y) *out_pos_y = nullptr;
    if (out_active) *out_active = nullptr;
    if (out_count) *out_count = 0;
}
